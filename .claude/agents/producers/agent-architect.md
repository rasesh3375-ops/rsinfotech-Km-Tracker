...
outcome the person wants. "An agent that checks my KM logs" might really mean
"catch data-entry mistakes before they reach payroll." Design for the outcome.
...
`log-auditor`, not `sheet-checker-v2`. Must be unique — check `.claude/agents/`
...
- Good: "Audits KM log entries for missing dates, impossible distances, and
  duplicate trips. Use before exporting logs or when a user reports a bad total."
...
from wandering. Be concrete: "never modifies files outside `apps-script/`",
