...
import { runAgent, providerConfig, PROVIDER } from './llm.js';
...
export async function runProducers({ request, signal }) {
  const p = providerConfig();
  const spec = await runAgent({
    system: ARCHITECT,
    user: `Design an agent for this request:\n\n${request}`,
    model: p.smart, temperature: 0.3, signal
  });
  ...
  const review = await runAgent({
    ...
    model: p.smart, temperature: 0.2, signal
  });
...
export async function runResearchers({ topic = 'sweep', signal }) {
  const p = providerConfig();
  ...
  const findings = await runAgent({
    ...
    tools: researchTools,
    model: p.smart, temperature: 0.3, maxSteps: 8, signal
  });

  const analysis = await runAgent({
    system: ANALYST,
    user: `Convert these findings into decisions.\n\n${findings.text}`,
    model: p.smart, temperature: 0.4, signal
  });
...
export async function runWatchdog({ signal } = {}) {
  ...
  try {
    const p = providerConfig();
    checks.push({ check: 'llm-config', ok: true, detail: `${PROVIDER} configured, model ${p.smart}` });
  } catch (err) {
  ...
  return [
    {
      ...
      run: async ({ request }) => (await runProducers({ request, signal })).spoken
    },
    {
      schema: {
        type: 'function',
      ...
      run: async ({ topic }) => {
        const r = await runResearchers({ topic: topic || 'sweep', signal });
...
export async function runHub({ message, sessionId = 'default', signal }) {
  const p = providerConfig();
  ...
  return { reply: result.text, toolsUsed: result.toolsUsed, provider: PROVIDER };
}
